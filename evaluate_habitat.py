...
from habitat_nav_env import HabitatNavEnv
...
from habitat_nav_env import (
    HabitatNavEnv, CMD_NAMES, CMD_FORWARD, CMD_LEFT, CMD_RIGHT, CMD_STOP,
...
def load_model_and_env(seed: int = 42):
    ...
    raw_env = make_vec_env(
        lambda: Monitor(HabitatNavEnv(num_obstacles=8, random_seed=seed)),
        n_envs=1,
        seed=seed,
    )
...
def get_base_env(vec_env) -> HabitatNavEnv:
    """Return the underlying HabitatNavEnv from a VecEnv wrapper stack."""
...
class PolicyVisualizer:
    ...
    def update(self, env: HabitatNavEnv, obs: np.ndarray,
...
def compare(n_episodes: int = 10):
    ...
    def run_episodes(use_model: bool, n: int):
        env = HabitatNavEnv(num_obstacles=8, random_seed=7)
        if use_model:
...
def save_gif(output_path: str = "policy_episode.gif", max_steps: int = 300):
    ...
    render_env = HabitatNavEnv(render_mode="rgb_array", num_obstacles=8,
                                random_seed=5)
    render_env.reset()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate trained PPO policy on HabitatNavEnv"
    )
    parser.add_argument(
